# Runbook: attivare Google Drive come storage documenti e importare le 14 fatture Acme
...
fatture Acme (spec `docs/superpowers/specs/2026-09-04-slice-9-import-storico-e-google-drive-design.md`
...
`docs/superpowers/notes/2026-09-04-import-acme-runbook.md` — questo runbook lo sostituisce
...
### 6. Importare le 14 fatture Acme, dalla cartella Drive `Fatture`
...
3. Costruire il payload `InvoiceImport` come nel runbook Acme (righe del dataset,
...
4. Chiamare `import_issued_invoice` (MCP, server `pigrocrm`) o `POST /api/invoices/import`
   ...
   successiva — stessa disciplina del runbook Acme passo 4.

Al termine, dichiarare i buchi 1, 4, 6 con `declare_invoice_register_gaps` e verificare
come nel runbook Acme (§5): `list_invoices`, `invoice_counters.ultimo_numero = 17`,
