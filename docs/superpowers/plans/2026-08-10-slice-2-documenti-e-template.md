...
**Goal:** Give customers and deals a versioned document store, a template engine that turns Markdown into offers with context-correct escaping, a Pandoc→Typst PDF renderer, offer states, pluggable storage (local filesystem and Google Drive), a configurable emitter profile replacing the hardcoded issuer data, and a one-shot Attio importer.
...
├── render/
│   ├── assets/                        # carried from the previous system: pandoc-template.typst, header.typ.j2, media/
...
├── importers/attio.py                 # one-shot importer
└── cli.py                             # MODIFIED: `pigrocrm import-attio`
...
Create `packages/core/src/pigrocrm/core/render/assets/header.typ.template` — this is the previous system's `offer/header.typ` with every hardcoded issuer datum replaced by a placeholder the same engine fills in (`{{emittente.*}}`), which is exactly what the emitter profile exists for:
...
The cost table becomes a loop, which is the point of having `#each` at all — the previous system needed one template per number of cost lines:
...
/**
 * The single row that replaces the issuer data the previous system hardcoded into `header.typ`.
 *
...
# Phase 6 — The Attio importer

### Task 19: `pigrocrm import-attio` — one-shot, idempotent, dry-run by default

**Files:**
- Create: `packages/core/src/pigrocrm/core/importers/__init__.py`
- Create: `packages/core/src/pigrocrm/core/importers/attio.py`
- Modify: `packages/core/src/pigrocrm/core/cli.py:40-50`
- Test: `packages/core/tests/test_importer_attio.py`

**Interfaces:**
- Consumes: `CustomerService`, `PersonService`, `DealService`, `PipelineService`, `FieldDefinitionService` and their Create/Update schemas; `Actor`; `slugify_key` from `pigrocrm.core.fields.schemas`.
- Produces:
  - `@dataclass(frozen=True) class ImportPlan: create_customers: int; update_customers: int; create_people: int; update_people: int; create_deals: int; update_deals: int; new_custom_fields: tuple[tuple[str, str], ...]; warnings: tuple[str, ...]`
  - `NATIVE_BY_ATTIO_COLUMN: dict[str, dict[str, str]]` — Attio column header → native field, per record type
  - `read_export(path: Path) -> dict[str, list[dict[str, str]]]`
  - `class AttioImporter:` `__init__(self, session: Session, actor: Actor)`, `plan(self, export: dict[str, list[dict[str, str]]]) -> ImportPlan`, `apply(self, export: dict[str, list[dict[str, str]]]) -> ImportPlan`
  - `import_attio(path: str, apply_changes: bool) -> int` in `cli.py`, wired to `pigrocrm import-attio <path> [--apply]`

- [ ] **Step 1: Write the failing test**

```python
# packages/core/tests/test_importer_attio.py
import csv
from pathlib import Path
...
from pigrocrm.core.fields.service import FieldDefinitionService
from pigrocrm.core.importers.attio import AttioImporter, read_export
...
COMPANIES = [
    {
        ...
        "VAT number": "14518240966",
        "Email": "someone@example.com",
        "Phone": "+39 02 1234567",
        "Street address": "Via Roma 1",
        "Postal code": "20053",
        "City": "Milano",
        "Country": "IT",
        "Employee range": "11-50",
        "Attio Record ID": "rec_company_1",
    },
    {
        "Name": "Beta S.p.A.",
...
PEOPLE = [
    {
        ...
        "Phone numbers": "+39 02 1234567",
        "Job title": "CTO",
        "Company": "ACME S.r.l.",
        "LinkedIn": "https://linkedin.com/in/ivansala",
        "Seniority": "Executive",
        "Attio Record ID": "rec_person_1",
    }
]
...
DEALS = [
    {
        ...
        "Deal owner": "someone@example.com",
        "Attio Record ID": "rec_deal_1",
    }
]


def _write_export(root: Path) -> Path:
    for name, rows in (("companies", COMPANIES), ("people", PEOPLE), ("deals", DEALS)):
...
def test_read_export_reads_the_three_csv_files(tmp_path: Path) -> None:
    export = read_export(_write_export(tmp_path))
    assert [row["Name"] for row in export["companies"]] == ["ACME S.r.l.", "Beta S.p.A."]
    assert export["people"][0]["First name"] == "Ivan"
    assert export["deals"][0]["Name"] == "Advisory 2026"


def test_read_export_of_a_missing_directory_is_an_error(tmp_path: Path) -> None:
    with pytest.raises(FileNotFoundError):
        read_export(tmp_path / "non-esiste")


def test_plan_writes_nothing(db_session: Session, tmp_path: Path) -> None:
    """The dry run is the default and it must be a true dry run: nothing is created,
    not even the custom field definitions it reports it would create."""
    export = read_export(_write_export(tmp_path))
    plan = AttioImporter(db_session, ADMIN).plan(export)
    assert plan.create_customers == 2
    assert plan.create_people == 1
    assert plan.create_deals == 1
    assert CustomerService(db_session).list(CustomerListQuery(), ADMIN).items == []
    assert FieldDefinitionService(db_session).list("customer") == []


def test_plan_reports_the_custom_fields_it_would_create(db_session: Session, tmp_path: Path) -> None:
    export = read_export(_write_export(tmp_path))
    plan = AttioImporter(db_session, ADMIN).plan(export)
    keys = dict(plan.new_custom_fields)
    # Unrecognised columns become custom fields -- exactly the use case custom fields
    # exist for (spec 8). "Attio Record ID" is not one of them: it is the import's own
    # correlation key, not the user's data.
    assert keys["customer:employee_range"] == "Employee range"
    assert keys["person:seniority"] == "Seniority"
    assert "customer:attio_record_id" not in keys


def test_apply_creates_customers_people_and_deals(db_session: Session, tmp_path: Path) -> None:
    export = read_export(_write_export(tmp_path))
    AttioImporter(db_session, ADMIN).apply(export)
    customers = CustomerService(db_session).list(CustomerListQuery(), ADMIN).items
    assert sorted(c.ragione_sociale for c in customers) == ["ACME S.r.l.", "Beta S.p.A."]
    acme = next(c for c in customers if c.ragione_sociale == "ACME S.r.l.")
    assert acme.partita_iva == "14518240966"
    assert acme.comune == "Milano"
    assert acme.custom_fields["employee_range"] == "11-50"
    people = PersonService(db_session).list(PersonListQuery(), ADMIN).items
    assert [p.nome for p in people] == ["Ivan"]
    assert people[0].customer_id == acme.id


def test_apply_creates_the_custom_field_definitions_it_needs(
    db_session: Session, tmp_path: Path
) -> None:
    export = read_export(_write_export(tmp_path))
    AttioImporter(db_session, ADMIN).apply(export)
    keys = {f.key for f in FieldDefinitionService(db_session).list("customer")}
    assert "employee_range" in keys


def test_running_twice_produces_the_same_state(db_session: Session, tmp_path: Path) -> None:
    """Spec 11 criterion 6. A second pass updates and nothing else."""
    export = read_export(_write_export(tmp_path))
    importer = AttioImporter(db_session, ADMIN)
    importer.apply(export)
    first = {
        (c.ragione_sociale, c.partita_iva)
        for c in CustomerService(db_session).list(CustomerListQuery(), ADMIN).items
    }
    second_plan = importer.apply(export)
    second = {
        (c.ragione_sociale, c.partita_iva)
        for c in CustomerService(db_session).list(CustomerListQuery(), ADMIN).items
    }
    assert first == second
    assert second_plan.create_customers == 0
    assert second_plan.update_customers == 2
    assert second_plan.create_people == 0
    assert second_plan.create_deals == 0


def test_a_second_pass_picks_up_a_changed_value(db_session: Session, tmp_path: Path) -> None:
    root = _write_export(tmp_path)
    importer = AttioImporter(db_session, ADMIN)
    importer.apply(read_export(root))
    with (root / "companies.csv").open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(COMPANIES[0]))
        writer.writeheader()
        writer.writerows([{**COMPANIES[0], "Phone": "+39 333 1234567"}, COMPANIES[1]])
    importer.apply(read_export(root))
    acme = next(
        c
        for c in CustomerService(db_session).list(CustomerListQuery(), ADMIN).items
        if c.ragione_sociale == "ACME S.r.l."
    )
    assert acme.telefono == "+39 333 1234567"


def test_a_row_with_no_name_is_skipped_and_reported(db_session: Session, tmp_path: Path) -> None:
    root = tmp_path
    with (root / "companies.csv").open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=["Name", "Attio Record ID"])
        writer.writeheader()
        writer.writerow({"Name": "  ", "Attio Record ID": "rec_x"})
    for name in ("people", "deals"):
        with (root / f"{name}.csv").open("w", encoding="utf-8", newline="") as handle:
            handle.write("Name\n")
    plan = AttioImporter(db_session, ADMIN).plan(read_export(root))
    assert plan.create_customers == 0
    assert any("senza nome" in warning for warning in plan.warnings)


def test_a_deal_pointing_at_an_unknown_company_is_skipped_and_reported(
    db_session: Session, tmp_path: Path
) -> None:
    root = tmp_path
    with (root / "companies.csv").open("w", encoding="utf-8", newline="") as handle:
        handle.write("Name,Attio Record ID\n")
    with (root / "people.csv").open("w", encoding="utf-8", newline="") as handle:
        handle.write("First name\n")
    with (root / "deals.csv").open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(DEALS[0]))
        writer.writeheader()
        writer.writerows(DEALS)
    plan = AttioImporter(db_session, ADMIN).plan(read_export(root))
    assert plan.create_deals == 0
    assert any("ACME S.r.l." in warning for warning in plan.warnings)


def test_a_malformed_vat_number_is_reported_rather_than_aborting_the_import(
    db_session: Session, tmp_path: Path
) -> None:
    root = tmp_path
    with (root / "companies.csv").open("w", encoding="utf-8", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(COMPANIES[0]))
        writer.writeheader()
        writer.writerows([{**COMPANIES[0], "VAT number": "123"}, COMPANIES[1]])
    for name in ("people", "deals"):
        with (root / f"{name}.csv").open("w", encoding="utf-8", newline="") as handle:
            handle.write("Name\n")
    plan = AttioImporter(db_session, ADMIN).apply(read_export(root))
    # One bad row must not cost the other 499. It is reported and skipped.
    assert plan.create_customers == 1
    assert any("ACME S.r.l." in warning and "partita_iva" in warning for warning in plan.warnings)
    names = [c.ragione_sociale for c in CustomerService(db_session).list(CustomerListQuery(), ADMIN).items]
    assert names == ["Beta S.p.A."]
```

- [ ] **Step 2: Run test to verify it fails**

Run: `cd packages/core && uv run pytest tests/test_importer_attio.py -v`
Expected: FAIL with `ModuleNotFoundError: No module named 'pigrocrm.core.importers'`

- [ ] **Step 3: Write the importer**

```python
# packages/core/src/pigrocrm/core/importers/__init__.py
```

(empty file)

```python
# packages/core/src/pigrocrm/core/importers/attio.py
"""One-shot import from an Attio CSV export. Not a sync.

Attio is removed as a system of record (spec 2); what remains is this, run once by a
person migrating, from the command line. It is idempotent by design -- a second pass
updates and nothing else (spec 11 criterion 6) -- and it writes nothing at all unless
asked with `--apply`.

Correlation is by name, not by Attio's own record id: a `customers` row has nowhere to
put a foreign identifier, and adding a column for a system this product is deleting
would be the wrong trade. `Attio Record ID` is therefore read (so it is not mistaken
for user data and turned into a custom field) and otherwise ignored. The practical
consequence, documented rather than hidden: renaming a company in Attio between two
passes creates a second customer instead of renaming the first. For a one-shot
migration that is the right cost.
"""
...
from pigrocrm.core.fields.schemas import EntityType, FieldDefinitionCreate, slugify_key
from pigrocrm.core.fields.service import FieldDefinitionService
from pigrocrm.core.people.schemas import PersonCreate, PersonListQuery, PersonUpdate
from pigrocrm.core.people.service import PersonService

# The three files an Attio CSV export produces, and the columns this importer knows how
# to place on a native column. Everything else in a file becomes a custom field.
#
# `Attio Record ID` is deliberately listed with an empty target on every record type:
# that marks it "recognised, but not ours", so it is neither stored on a column nor
# turned into a custom field nobody asked for.
NATIVE_BY_ATTIO_COLUMN: dict[str, dict[str, str]] = {
    "companies": {
        "Name": "ragione_sociale",
        "VAT number": "partita_iva",
        "Tax ID": "codice_fiscale",
        "Email": "email",
        "Phone": "telefono",
        "Domains": "sito_web",
    ...
    "people": {
        ...
        "Job title": "ruolo",
        "LinkedIn": "linkedin",
        "Company": "",  # resolved to customer_id, not stored as text
        "Attio Record ID": "",
    },
    "deals": {
        "Name": "nome",
        "Value": "valore_previsto",
        "Close date": "data_chiusura_prevista",
        "Company": "",  # resolved to customer_id
        "Stage": "",  # resolved to pipeline_stage_id
        "Deal owner": "",  # no user mapping exists; reported, not guessed
        "Attio Record ID": "",
    },
}

FILES: tuple[str, ...] = ("companies", "people", "deals")

ENTITY_BY_FILE: dict[str, EntityType] = {
    "companies": "customer",
    "people": "person",
    "deals": "deal",
}
...
@dataclass(frozen=True)
class ImportPlan:
    """What a run did, or would do. The dry run and the real run return the same shape,
    so the report a person reads before `--apply` is the report they get after."""

    create_customers: int = 0
    update_customers: int = 0
    create_people: int = 0
    update_people: int = 0
    create_deals: int = 0
    update_deals: int = 0
    new_custom_fields: tuple[tuple[str, str], ...] = ()
    warnings: tuple[str, ...] = ()


def read_export(path: Path) -> dict[str, list[dict[str, str]]]:
    """Reads `companies.csv`, `people.csv` and `deals.csv` from a directory.

    A missing directory raises; a missing *file* is an empty list, because an Attio
    workspace with no deals is an ordinary thing and refusing to import the customers
    over it would be unhelpful.
    """
    if not path.is_dir():
        raise FileNotFoundError(f"cartella di export non trovata: {path}")
    export: dict[str, list[dict[str, str]]] = {}
    for name in FILES:
...
class AttioImporter:
    def __init__(self, session: Session, actor: Actor) -> None:
        ...
        self.people = PersonService(session)
        self.deals = DealService(session)
        self.fields = FieldDefinitionService(session)

    # ---- shared ------------------------------------------------------------

    def _split_row(self, file: str, row: dict[str, str]) -> tuple[dict[str, Any], dict[str, Any]]:
        """`(native values, custom values)` for one CSV row.

        A column this importer does not recognise becomes a custom field, keyed by
        `slugify_key(header)` -- which is exactly the case custom fields exist for
        (spec 8). Blank cells are dropped entirely rather than written as `""`: an
        Attio export pads every row to the full column set, so an empty cell means
        "this record has no value", not "clear this field".
        """
        mapping = NATIVE_BY_ATTIO_COLUMN[file]
        native: dict[str, Any] = {}
        custom: dict[str, Any] = {}
        for header, value in row.items():
            ...
            if header in mapping:
                ...
                if target:
                    native[target] = value
                continue
            custom[slugify_key(header)] = value
        return native, custom

    def _custom_field_labels(self, export: dict[str, list[dict[str, str]]]) -> dict[str, str]:
        """`"{entity}:{key}" -> original header`, for every column that needs a
        definition and does not have one yet."""
        needed: dict[str, str] = {}
        for file in FILES:
            entity = ENTITY_BY_FILE[file]
            existing = {f.key for f in self.fields.list(entity, include_archived=True)}
            mapping = NATIVE_BY_ATTIO_COLUMN[file]
            for row in export[file]:
                for header, value in row.items():
                    if not value or header in mapping:
                        continue
                    key = slugify_key(header)
                    if key and key not in existing:
                        needed[f"{entity}:{key}"] = header
        return needed

    def _ensure_custom_fields(self, needed: dict[str, str]) -> None:
        for qualified, label in needed.items():
            entity, _, key = qualified.partition(":")
            self.fields.create(
    ...
    def _customer_ids_by_name(self) -> dict[str, Any]:
        page = self.customers.list(CustomerListQuery(limit=200), self.actor)
        found = {c.ragione_sociale: c.id for c in page.items}
        cursor = page.next_cursor
        while cursor is not None:
            page = self.customers.list(CustomerListQuery(limit=200, cursor=cursor), self.actor)
            found.update({c.ragione_sociale: c.id for c in page.items})
            cursor = page.next_cursor
        return found

    def _person_ids_by_email(self) -> dict[str, Any]:
        page = self.people.list(PersonListQuery(limit=200), self.actor)
        found = {p.email: p.id for p in page.items if p.email}
        cursor = page.next_cursor
        while cursor is not None:
            page = self.people.list(PersonListQuery(limit=200, cursor=cursor), self.actor)
            found.update({p.email: p.id for p in page.items if p.email})
            cursor = page.next_cursor
        return found

    def _deal_ids_by_name(self) -> dict[str, Any]:
        page = self.deals.list(DealListQuery(limit=200), self.actor)
        found = {d.nome: d.id for d in page.items}
        cursor = page.next_cursor
        while cursor is not None:
            page = self.deals.list(DealListQuery(limit=200, cursor=cursor), self.actor)
            found.update({d.nome: d.id for d in page.items})
            cursor = page.next_cursor
        return found

    # ---- the run ------------------------------------------------------------

    def plan(self, export: dict[str, list[dict[str, str]]]) -> ImportPlan:
        """A true dry run: it writes nothing at all, not even the field definitions it
        reports it would create."""
        return self._run(export, write=False)

    def apply(self, export: dict[str, list[dict[str, str]]]) -> ImportPlan:
        return self._run(export, write=True)

    def _run(self, export: dict[str, list[dict[str, str]]], *, write: bool) -> ImportPlan:
        warnings: list[str] = []
        needed_fields = self._custom_field_labels(export)
        if write:
            self._ensure_custom_fields(needed_fields)
        ...
        existing_customers = self._customer_ids_by_name() if write else {}
        for row in export["companies"]:
            native, custom = self._split_row("companies", row)
            name = native.get("ragione_sociale")
            if not name:
                warnings.append(f"riga companies senza nome, saltata: {row.get('Attio Record ID', '?')}")
                continue
            if not write:
                counts["create_customers"] += 1
                continue
            existing_id = existing_customers.get(name)
            try:
                if existing_id is None:
                    created = self.customers.create(
                        CustomerCreate(**native, custom_fields=custom), self.actor
                    )
                    existing_customers[name] = created.id
                    counts["create_customers"] += 1
                else:
                    self.customers.update(
                        existing_id, CustomerUpdate(**native, custom_fields=custom), self.actor
                    )
                    counts["update_customers"] += 1
            except DomainError as exc:
                # One bad row must not cost the other 499. The service already rolled
                # back its own failed transaction (every commit here is per-record), so
                # the session is usable for the next one.
                warnings.append(f"cliente '{name}' saltato: {exc.message}")

        existing_people = self._person_ids_by_email() if write else {}
        for row in export["people"]:
            native, custom = self._split_row("people", row)
            if not native.get("nome"):
                warnings.append("riga people senza nome, saltata")
                continue
            company = row.get("Company", "")
            customer_id = existing_customers.get(company) if company else None
            if company and customer_id is None and write:
                warnings.append(f"persona '{native['nome']}': cliente '{company}' non trovato")
            if not write:
                counts["create_people"] += 1
                continue
            email = native.get("email")
            existing_id = existing_people.get(email) if email else None
            try:
                if existing_id is None:
                    created_person = self.people.create(
                        PersonCreate(**native, customer_id=customer_id, custom_fields=custom),
                        self.actor,
                    )
                    if email:
                        existing_people[email] = created_person.id
                    counts["create_people"] += 1
                else:
                    self.people.update(
                        existing_id,
                        PersonUpdate(**native, customer_id=customer_id, custom_fields=custom),
                        self.actor,
                    )
                    counts["update_people"] += 1
            except DomainError as exc:
                warnings.append(f"persona '{native['nome']}' saltata: {exc.message}")

        existing_deals = self._deal_ids_by_name() if write else {}
        for row in export["deals"]:
            native, custom = self._split_row("deals", row)
            name = native.get("nome")
            company = row.get("Company", "")
            customer_id = (existing_customers if write else self._customer_ids_by_name()).get(company)
            if not name:
                warnings.append("riga deals senza nome, saltata")
                continue
            if customer_id is None:
                # A deal without a customer has no economic meaning and `Deal.customer_id`
                # is NOT NULL -- so this is skipped, loudly, rather than invented.
                warnings.append(f"deal '{name}' saltato: cliente '{company}' non trovato")
                continue
            if row.get("Deal owner"):
                warnings.append(
                    f"deal '{name}': proprietario '{row['Deal owner']}' non assegnato "
                    "(nessuna corrispondenza fra utenti Attio e utenti PigroCRM)"
                )
            if not write:
                counts["create_deals"] += 1
                continue
            existing_id = existing_deals.get(name)
            try:
                if existing_id is None:
                    created_deal = self.deals.create(
                        DealCreate(**native, customer_id=customer_id, custom_fields=custom),
                        self.actor,
                    )
                    existing_deals[name] = created_deal.id
                    counts["create_deals"] += 1
                else:
                    self.deals.update(
                        existing_id, DealUpdate(**native, custom_fields=custom), self.actor
                    )
                    counts["update_deals"] += 1
            except DomainError as exc:
                warnings.append(f"deal '{name}' saltato: {exc.message}")

        return ImportPlan(
            **counts,
            new_custom_fields=tuple(sorted(needed_fields.items())),
...
In `packages/core/src/pigrocrm/core/cli.py`, add the function above `main` and the sub-parser inside it:

```python
def import_attio(path: str, apply_changes: bool) -> int:
    """One-shot import from an Attio CSV export. Dry run unless `--apply` is given."""
    engine = create_engine_from_settings(get_settings())
    with session_factory(engine)() as session:
        importer = AttioImporter(session, Actor.system())
        try:
            export = read_export(Path(path))
        except FileNotFoundError as exc:
            print(str(exc), file=sys.stderr)
            return 1
        plan = importer.apply(export) if apply_changes else importer.plan(export)

    verb = "Importati" if apply_changes else "Da importare"
    print(f"{verb}: {plan.create_customers} clienti nuovi, {plan.update_customers} aggiornati")
    print(f"{verb}: {plan.create_people} persone nuove, {plan.update_people} aggiornate")
    print(f"{verb}: {plan.create_deals} deal nuovi, {plan.update_deals} aggiornati")
    for qualified, label in plan.new_custom_fields:
        print(f"  campo personalizzato: {qualified} ('{label}')")
    for warning in plan.warnings:
        print(f"  attenzione: {warning}", file=sys.stderr)
    if not apply_changes:
        print("\nNessuna modifica scritta. Riesegui con --apply per applicare.")
    return 0
```
...
```python
    attio = sub.add_parser("import-attio", help="Importa un export CSV di Attio (una tantum)")
    attio.add_argument("path", help="Cartella con companies.csv, people.csv, deals.csv")
    attio.add_argument(
        "--apply",
        action="store_true",
        help="Scrive davvero. Senza questo flag stampa solo cosa farebbe.",
    )
```

```python
    if args.command == "import-attio":
        return import_attio(args.path, args.apply)
```
...
Run: `cd packages/core && uv run pytest -q && cd ../../apps/api && uv run pytest -q && cd ../mcp && uv run pytest -q`
Expected: PASS everywhere.

- [ ] **Step 7: Commit**

```bash
git add packages/core/src/pigrocrm/core/importers packages/core/src/pigrocrm/core/cli.py \
        packages/core/tests/test_importer_attio.py
git commit -m "feat(importers): one-shot idempotent Attio import with dry run"
```
...
6. **The Attio importer, run twice, produces the same state.** `test_running_twice_produces_the_same_state` (Task 19).
...
| §8 Attio importer, idempotent, dry run, auto custom fields | 19 |
