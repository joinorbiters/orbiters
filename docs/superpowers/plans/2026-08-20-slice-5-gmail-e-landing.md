...
3. **The spec says `activities.entity_type` is "an open value by project"; `fields/schemas.py` says it is a closed `Literal`.** Residuo **R13** already records that this promise has been checked and disproved three times. But the two are about different columns: `ActivityService.record` takes `entity_type: str` and truncates it to the column width (`activities/service.py:60`) without consulting any `Literal`, so `entity_type='google_account'` genuinely needs no migration and no schema change. `fields.EntityType` is the *custom-field* entity type, and Gmail adds no custom fields. **Resolution:** the spec is right for `activities` and `fields/schemas.py` is not touched. Recorded here so nobody spends a fourth investigation on it.
