...
**Ambito:** (a) registrare nel CRM le fatture già emesse con il gestionale precedente (Acme),
...
Acme e nei PDF su Drive. Senza di esse lo scadenziario (slice 8), il P&L (slice 4) e il
...
|---|---|---|
| Come caricare le 14 fatture Acme | **Import storico dedicato**: la fattura entra come `emessa`, marcata importata, con numero e data originali, senza XML/PDF generati | Solo contatore + note (niente scadenziario); re-emissione via proforma (numeri nuovi e XML duplicati verso lo SdI) |
...
|---|---|---|
| `importata_da` | `String(20)` null | `'esterno'` per questo slice (era `'acme'`, rinominato dalla migrazione 0029: il valore raggiunge API e schermo, quindi non nomina nessun prodotto). `NULL` = emessa da PigroCRM. È il campo che dice «di questa fattura il CRM non ha prodotto né XML né PDF» |
...
da Acme: l'XML è quello già trasmesso allo SdI dal gestionale precedente». Il PDF è
...
4. **I buchi si dichiarano.** Nel registro Acme del 2026 mancano 1, 4 e 6. L'import **non**
...
causale?,                                      # es. "207571/0426/Consulenza AI CTO Safely A2A"
...
trasmessa_esternamente_il?,                    # data di trasmissione SdI da Acme
...
(vincolo `ck_invoices_riferimento_only_on_proforma`), e la descrizione che Acme stampava
...
| Frontend | Fuori ambito: il titolare importa via MCP o REST. La lista fatture mostra il badge «importata da Acme» e nasconde i pulsanti XML/rigenera su quelle righe |
...
registro Acme (screenshot del 4/09). Tutti e sei i clienti esistono già nel CRM; i PDF stanno nella cartella
`Fatture` del Drive humancraft. La fattura 5/00 (Acme) è «emessa e non consegnata» in Acme:
