...
def test_a_customer_filter_excludes_general_expenses_entirely(
    ...
    unscoped = AnalyticsService(db_session).period_pnl(MARCH, READER)
...
def test_each_quantity_is_attributed_by_its_own_date(
    ...
    # A general expense one day past the window, so `spese_generali` is subject to the
