...
double send comes back -- Acme's `emailSentCount` lived in a JSON file precisely because
...
def test_the_draft_body_is_the_template_and_not_a_string_in_the_source(
    ...
    assert IBAN in draft.body_markdown
    # De-personalised in B2-7: Acme's signature was duplicated verbatim in both of its
...
def test_the_amount_is_the_invoices_own_frozen_figure(db_session: Session) -> None:
    ...
    of the invoice does not carry is a demand they are right to ignore -- and Acme
...
def test_a_missing_iban_is_refused_and_never_becomes_a_placeholder(
    ...
    """Acme's `normalizeIban(iban) || 'IBAN_PAGAMENTO'` shipped the placeholder to the
