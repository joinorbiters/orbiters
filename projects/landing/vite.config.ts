...
/**
 * nginx maps `/privacy` to `/privacy.html` in production (projects/landing/deploy/nginx.conf).
...
function extensionlessHtml(): Plugin {
  ...
  return {
    name: 'landing-extensionless-html',
...
const apiUrl = process.env.LANDING_API_URL ?? 'http://localhost:8000'
...
export default defineConfig({
  ...
  // API so the form can be exercised locally; LANDING_API_URL points it elsewhere.
