...
E infine: questo slice ospita l'ultimo pezzo di logica di Acme che vive ancora nel browser. Lo slice
...
## 2. Cosa si porta da Acme, e cosa si riscrive
...
Le fonti lette per intero: `.reference-acme/website/vite.config.js` righe 1076-1265 (modello e
persistenza delle voci), 3171-3362 (API `/api/time-tracking`), `.reference-acme/offer/template-time-tracking.typ`,
e `.reference-acme/website/src/App.jsx` righe 1783-1963 (la vista `project-costs`, che è il P&L di
Acme).
...
slice; un'ora agganciata a niente è la voce fantasma che Acme produce e che nessun export mostra
...
regime sono». Default dal profilo attuale di Acme: `67.00`, `5.00`, `26.07`.
...
difetto di Acme al §2.2, ed è la stessa conclusione dello slice 3 §6.2 su `data_emissione`.
...
ed è il difetto che il P&L di Acme ha per la fiscalità (§8).

---

## 8. Il calcolo fiscale, spostato dove ha senso

Acme calcola, dentro `App.jsx` e per singola offerta:
...
correzione che il livello giusto non è quello che Acme aveva scelto.
...
versioning e il suo hash. Acme teneva l'allegato come base64 dentro il JSON dei costi
