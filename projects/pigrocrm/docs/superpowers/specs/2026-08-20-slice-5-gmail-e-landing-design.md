...
## 2. Cosa si porta da Acme, e cosa si rifà

Acme manda email da Gmail e manda solleciti già oggi, in produzione. Vale lo stesso criterio
...
| Elemento di Acme | Decisione | Perché |
|---|---|---|
...
Due difetti di Acme meritano più di una riga di tabella, perché sono esattamente i due problemi
...
**Il precedente tecnico da seguire non è in Acme, è in `packages/core/src/pigrocrm/core/storage/gdrive.py`**
...
| Una **firma email testuale** nell'`emitter_profile` | Slice 2 §4.4 — ma vedi il §10: l'`emitter_profile` **in corso di implementazione** ha `firma_key`, che è la chiave storage di un'immagine di firma, non un blocco di testo | La firma tornerebbe hardcodata nel sorgente, come in Acme, e duplicata in ogni template che la usa |
...
Tre regole sulla costruzione dell'RFC822, tutte e tre imparate dai difetti di Acme elencati al §2:
...
3. **Il corpo si dichiara `quoted-printable` o `base64`, con `charset="UTF-8"`. Mai `7bit`.** Acme
...
**(b) Non sappiamo.** Timeout, connessione caduta, risposta persa — oppure, come in Acme oggi,
...
Il confronto con Acme è la giustificazione di ognuna delle quattro condizioni: là il sollecito è
...
**E c'è un segnale che Acme non poteva avere.** Da quando il CRM legge la posta (§4), la lista
...
### 9.2 Cosa si porta dal `website/` di Acme — e la correzione necessaria

Prima di progettare, una correzione di fatto, perché cambia cosa c'è da studiare:
**`.reference-acme/website/` non è una landing page.** È l'intera applicazione Acme — una SPA
...
| **Movimento** | Il `rise` di Acme, accorciato: `IntersectionObserver` che aggiunge una classe, `opacity` + `translateY(12px)`, **`320ms`** invece di `0.6s`, `cubic-bezier(.2,.7,.2,1)`, sfalsamento `60ms`. **Nessun parallasse, nessuna trasformazione legata allo scroll**: combattono con chi legge e su un telefono costano. Tutto dentro `@media (prefers-reduced-motion: reduce)` che le annulla — la guardia c'era già in Acme e si porta |
...
| **Carattere** | `Outfit` e nient'altro, dal **medesimo** woff2 già in `apps/web/src/assets/fonts/`, servito dal proprio origin. Riusato, non ri-aggiunto, e mai da CDN (§9.2). Tipografia fluida con `clamp()`; sopratitoli `uppercase` a `letter-spacing: .24em` / `.75rem`, portati da Acme |
...
12. Un corpo con accenti, virgolette tipografiche ed emoji, e un oggetto con gli stessi: l'RFC822
    ...
    7bit` su testo non ASCII, in nessun ramo. È il difetto vivo di Acme (§2), verificato per non
    riprodurlo.
...
25. Ogni pagina della landing ha `title`, `meta description`, `lang="it"` e Open Graph **propri e
    ...
    «AI optimization platform», la copia di un altro prodotto rimasta nello scaffold di Acme
