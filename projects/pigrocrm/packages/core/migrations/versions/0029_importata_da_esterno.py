"""invoices.importata_da: 'acme' becomes 'esterno'
...
The downgrade puts `'acme'` back rather than dropping to `NULL`: a database rolled back
...
rename is about, and `WHERE importata_da = 'acme'` leaves every `NULL` untouched.
...
def upgrade() -> None:
    _rename("acme", "esterno")


def downgrade() -> None:
    _rename("esterno", "acme")
