...
|---|---|---|
| `palette.css` | Six colours and `--font-sans`, in a Tailwind `@theme` block | The CRM's `tokens.css` imports it; the landing extracts it at build time |
...
The CRM is built with Tailwind and the landing deliberately has none, so they cannot
share a stylesheet: the application consumes `@theme` and generates utilities from it,
while the landing reads the same declarations as text and injects them as plain custom
...
`--radius`. The landing's visual system is square by design: tiles, hard edges and a
...
26 semantic re-exports of `@theme inline`, back when the landing extracted them out of
...
contrast ratios against the values, the landing's `landing-tokens.test.ts` checks that
no stylesheet restates them, and `palette-plugin.test.ts` asserts the exact set of
seven tokens the landing receives.
